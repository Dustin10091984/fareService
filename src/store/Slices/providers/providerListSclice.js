...
export const getProviderList = ({ search, params, loadMore }) => async dispatch => {
    try {
        dispatch(getProvider({ data: { error: false, loading: true } }));
        await axios({
            method: 'get',
            url: process.env.REACT_APP_API_BASE_URL + `/api/user/services/provider-list${search}`,
            params
        }).then((response) => {
            //handle success
        ...
        dispatch(getProvider({ data: { error: true, loading: false, message: "something went wrong!" }, loadMore }));
    }
}