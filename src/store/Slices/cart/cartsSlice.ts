      ...
      if (action.payload.data) {
        if (state?.list?.cart) {
          return {
            ...
            list: {
              ...state.list,
              cart: [action.payload.data, ...state.list.cart],
...
export const updateQuantity = (data) => async (dispatch) => {
  if (data.id && data.quantity) {
    dispatch(
      helperAxios("PUT", `${url}/${data.id}`, updateCart, true, {
...
export const deleteCart = (id) => async (dispatch) => {
  dispatch(
    helperAxios("delete", `${url}/${id}`, remove, true, null, false, null)
  );
};
