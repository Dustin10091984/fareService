...
const { requestService, serviceRequestList, serviceRequestDetail } =
  requestServiceSlice.actions;
export const { serviceRequestListUpdate, serviceRequestDetailUpdate } =
  requestServiceSlice.actions;
 ...
 * @returns
 */
export const getServiceRequestList =
  ({ params }) =>
  async (dispatch) => {
    try {
      ...
      await axios({
        ...
        url: `${process.env.REACT_APP_API_BASE_URL}/api/user/service-request?${
          params || ""
        }`,
      })
        .then((response) => {
