...
export const Chat = ({ isChatOpen, ...props }) => {
  ...
  if (window.Echo && active?.userId) {
    window.Echo.channel(
      `newMessage-${active.userId}-${JSON.parse(localStorage?.user_data)?.id}`
      ...
      <MainContainer responsive className="border-0">
        <Sidebar position="left" scrollable={true}>
          <Search
            ...
            className="mx-4 rounded-pill border-1 bg-primary-light py-3 text-base"
            onChange={(value) => {
              setSearch(value);
            }}
          />
            ...
            {(() => {
              if (state?.provider_id == null) {
                return state?.providers
                  ?.filter((data) => {
                    return (
                      data?.provider?.first_name?.includes(search) ||
                      data?.provider?.last_name?.includes(search)
                    );
                  })
                  ?.map((data, index) => (
                    <Conversation
                      ...
                      //         ?.last_name
                      // }
                      info={moment(data?.create_at).format("LLL")}
                      onClick={() =>
                        setState({
                          ...state,
              ...
              return list
                ?.filter((serviceRequest) => {
                  return (
                    serviceRequest?.provider?.first_name?.includes(search) ||
                    serviceRequest?.provider?.last_name?.includes(search)
                  );
                })
                ?.map((serviceRequest, index) => {
                  if (serviceRequest?.provider?.id == state?.provider_id) {
                    return serviceRequest?.provider?.id == active?.userId &&
                      serviceRequest?.id == active?.orderId ? (
                      <Conversation
                        key={index}
                        name={
                          serviceRequest?.provider?.first_name
                            ? `${
                                serviceRequest?.provider?.first_name ||
                                serviceRequest?.type
        ...
        <ChatContainer>
          <ConversationHeader className="border-0 shadow-normal">
            <ConversationHeader.Back />
            <Avatar
              src={active?.image ? `${HOST}${active.image}` : image}
              name={
                active?.name
          ...
          {(() => {
            ...
            if (active?.userId && active?.orderId) {
              return (
                <MessageList
                  loading={
                    messageLoading == true && messagesdata?.data == undefined
                      ? true
                      : false
                  }
                  loadingMore={
                    messageLoading || (MessageError == true && false)
                  }
                  onYReachStart={
                ...
                >
                  {messagesdata?.current_page == messagesdata?.last_page &&
                    messagesdata?.data && <MessageSeparator content="End" />}
                  {messagesdata &&
          ...
          <MessageInput
            className="bg-primary-main items-center gap-4"
            placeholder={
